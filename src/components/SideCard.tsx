...
import { RoomWithReservations } from "~/server/api/routers/room";
  ...
  return (
    ...
    >
      ...
      <div className="mb-4 mt-2 w-fit rounded-lg bg-red-200 px-2 text-sm text-red-800">
        Ocupado los siguientes 30 minutos
      </div>
      <div className="mt-4 flex h-full flex-col overflow-hidden">
        ...
        <div className="hour-divider z-0 flex h-[50%] flex-col overflow-y-scroll rounded-b-md rounded-t-md">
          {selectedRoom?.Reservation.map((reservation) => (
            <SideHours
              key={reservation.id}
        ...
        <div className="mt-auto flex flex-col gap-2 ">
          <div className=" cursor-pointer rounded-md border-2 border-green-500 px-4 py-2 text-center  text-green-500">
            Programar Reserva
          </div>
          <div className=" cursor-pointer rounded-md bg-green-500 px-4 py-2 text-center font-bold text-white shadow-lg shadow-green-500/50">
            Reservar por 30m
          </div>
        </div>
      </div>
